...
snippetPageView.setAttribute('src', `${window.location.origin}/node/add/dsu_product`)
...
function showConsoleCopydeckBasicData () {
	copydeckBasicData = {
		"Product Title Local": copydeckData[4-step],
		"TTT URL Match": copydeckData[7-step],
		"GTIN Local": copydeckData[10-step],
		"bvID Local": copydeckData[8-step],
		"fpID Local": copydeckData[9-step],
		"Brand Local": copydeckData[11-step],
		"Pet Type Local": copydeckData[13-step],
		"Pack Sizes Local": copydeckData[16-step],
		"Headline Local": copydeckData[12-step] +' '+ copydeckData[45-step],
		"Description Final Local": copydeckData[18-step],
		"Feature 1 Local": copydeckData[20-step],
		"Feature 2 Local": copydeckData[22-step],
		"Feature 3 Local": copydeckData[24-step],
		"Feature 4 Local": copydeckData[26-step],
		"Feature 5 Local": copydeckData[28-step],
		"Feature 6 Local": copydeckData[30-step],
		"Feature 7 Local": copydeckData[32-step],
		"Feature 8 Local": copydeckData[34-step],
		"Feature 9 Local": copydeckData[36-step],
		"Composition Local": copydeckData[38-step],
		"Analytical Constituents Local": copydeckData[40-step],
		"Nutritional Additives Local": copydeckData[40-step],
		"Feeding Guidelines Local": copydeckData[43-step],
		"Food Type Local": copydeckData[44-step],
		"Range Local": copydeckData[46-step],
		"Category Local": copydeckData[48-step],
		"Lifestage Local": copydeckData[50-step],
		"Ingredients Local": copydeckData[52-step],
...
function externalTitleFormatter() {
	...
	const copydeckBrand = copydeckData[12-step].trim()

	return copydeckTitle ? copydeckTitle : ''
}

function bazaavoiceProductIDFormatter() {
	const copydeckId = copydeckData[8-step].replace(/[^0-9]/gm, '').trim()
...
function newsletterFormatter() {
	const copydecPetType = copydeckData[13-step] //old 16
...
function headLineFormatter() {
	const prodCategoryLocal = copydeckData[12-step] +' '+ copydeckData[45-step] //old 22

	return prodCategoryLocal ? prodCategoryLocal : ''
}

function highlightsFormatter() {
	const firstHighlightIndex = 20-step // old 36
	const lastHighlightIndex = 36-step // old 54
...
function productSizeFormatter() {
	const copydeckPackSize = copydeckData[16-step] //old 32
	let packSizes = []
...
function productOverviewFormatter() {
	const copydeckProdDesc1 = copydeckData[18-step].trim() //old 34
...
function ingredientsAndNutritionFormatter() {
	const copydeckIngredients = copydeckData[38-step].trim() //old 56
		? copydeckData[38-step].replace(/["♥]/gi, '')
		: ''
	const copydeckIngredientsNutritionalAdditives = copydeckData[40-step].trim() //old 60
		? copydeckData[40-step].replace(/["♥]/gi, '')
		: ''
...
function feedingGuideFormatter() {
	const copydeckFeedingGuide = copydeckData[43-step] //old 62
...
function categoriesFormatter() {
	const copydeckPetType = copydeckData[12] //old 16
	const copydeckFoodTypeLocal = copydeckData[47] //old 21
...
function urlAliasFormatter() {
	const urlAlias = copydeckData[7-step]
